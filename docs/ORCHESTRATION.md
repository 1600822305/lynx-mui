...
- **`styled(tag, options?)(...styles)`**(`system/styled.ts`)照 MUI/emotion 调用式:`styles` 可为 sx 对象或 `(props)=>sx`(props 带 `theme`+扁平化 `ownerState`),合并顺序 styled 样式 → `props.sx` → 显式 `props.style`(后者胜),再渲染基础元素。**降级**:不支持 tagged-template CSS 字符串(Lynx 无运行时 CSS 注入,这种写法手改);styled 样式里的 `&:hover/&:active` 由 sx 运行时解析但无 press host 自动跟踪(同非交互宿主上的裸 sx);不消费 `theme.components` 的 styleOverrides/variants(本库 theme 无此字段)。
